...
def send_welcome(message):
    teks = (
        ...
        "Cara pakai:\n"
        "1. `/tambahvisit YYYY-MM-DD HH:MM Nama_Resto`\n"
...
def add_visit(message):
    try:
        ...
        if len(parts) < 4:
            bot.reply_to(message, "⚠️ Format salah. Gunakan: /tambahvisit YYYY-MM-DD HH:MM Nama_Resto")
            return
        ...
        daily_visits = [v for v in visits if v['Tanggal'] == date_str]

        if len(daily_visits) >= 3:
            bot.reply_to(message, f"❌ Kuota visit tanggal {date_str} sudah penuh (Maks 3).")
            return

        if any(v['Jam'] == time_str for v in daily_visits):
        ...
        existing_post_dates = [p['TanggalPosting'] for p in posts]
...
def list_visit(message):
    ...
    for v in sorted(visits, key=lambda x: (x['Tanggal'], x['Jam'])):
...
def list_posting(message):
    ...
    # Urutkan berdasarkan tanggal posting
    for p in sorted(posts, key=lambda x: x['TanggalPosting']):
